...
class ProposalKnowledgeBase:
    ...
    def hybrid_search(self, query, k=5):
        ...
        results = [{"score": float(score), "document": self.documents[idx]} for score, idx in combined[:k]]
        return results
    
    def multi_hop_search(self, initial_query, k=5):
        first = self.hybrid_search(initial_query, k=3*k)
        refined_query = initial_query + " " + " ".join([r["document"]["content"][ :200] for r in first[:3]])
    ...
    def get_section_documents(self, section_name):
        return [self.documents[idx] for idx in self.section_map.get(section_name, [])]
    ...
    def extract_pricing_from_kb(self) -> List[int]:
        """
        Scan every markdown file in the KB for its 'Commercial Proposal' section,
        pull out all currency amounts (₹ or Rs.) and return them as ints.
        """
        prices = []
...
class SpecialistRAGDrafter:
    ...
    def generate_draft(self, section_name, rfp_section_content, relevant_kb_content, client_name):
        ...
        summarized_kb = summary_resp.choices[0].message.content
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                temperature=0.2  # low temp for grounding
            )
...
class EnhancedProposalGenerator:
    ...
    def generate_section(self, section_name, rfp_analysis, rfp_section_content,
        ...
        is_pricing = any(term in section_name.lower() for term in ["commercial", "pricing", "cost", "financial", "budget", "price"])

        # Extract pricing if needed
        pricing_summary = ""
        if is_pricing:
            ...
            if prices:
                ...
                pricing_summary = "No past pricing data found in KB."

        # Build the generation prompt (simplified excerpt)
        kb_items = "\n\n".join([
            f"--- {('Very Relevant' if item['score']>0.8 else 'Relevant')} ---\n"
            f"Topic: {item['document']['section_name']}\n{item['document']['content']}"
            for item in relevant_kb_content if item['score']>=0.5
        ])[:2000]

        pricing_block = f"\n\n## PRICING INSIGHT\n{pricing_summary}" if is_pricing else ""
        ...
        4. Avoid sensitive direct copy;
        5. Pricing only in commercial sections.
        {pricing_block}
        """
        try:
            ...
            if st.session_state.proposal_data and st.session_state.proposal_data["sections"]:
                ...
                for i, section_name in enumerate(section_names):
                    ...
                    with section_tabs[i]:
                        ...
                        with col2:
                            if st.button("Update Section", key=f"update_{section_name}"):
                                if feedback:
                                    try:
